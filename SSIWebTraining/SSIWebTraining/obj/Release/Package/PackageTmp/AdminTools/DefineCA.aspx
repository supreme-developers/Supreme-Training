                            ...
                            <div class="row ">
                                <div class="col-lg-8 col-lg-offset-2">
                                    <div class="row form-group">
                                        ...
                                        <div class="col-lg-6">
                                            <h4 class="pull-left">Training Type</h4>
                                            <telerik:RadComboBox runat="server" ID="RadComboBoxTrainingTypes" TabIndex="2" EmptyMessage="Select a Training Type" Style="width: 100%"
                                                InputCssClass="form-control input-lg" Skin="MetroTouch" ShowToggleImage="True"
                                                 DataValueField="TrainingTypeId" DataTextField="TrainingType">                                             
                                            </telerik:RadComboBox>
                                        </div>
                                    </div>
                                    ...
                                    <div class="row form-group">
                                        <div class="col-lg-12 pull-left">
                                            <h4 class="pull-left">Define Competency</h4>
                                    ...
                                    <div class="row form-group">
                                         <div id="Title" class="col-lg-12  w3-animate-right defineBy">
                                                <span class="text-blue pull-left">Job Titles </span>
                                                <telerik:RadComboBox runat="server" ID="RadComboBoxJobTitles" TabIndex="2" EmptyMessage="Select a Job Title" Style="width: 100%"
                                                    ...
                                                    Skin="MetroTouch" ShowToggleImage="True" CheckBoxes="true" DataTextField="JobTitle">
                                                </telerik:RadComboBox>
                                            </div>
                                            <div id="Role" class="col-lg-12  w3-animate-zoom defineBy">
                                                <span class="text-blue pull-left">Job Roles </span>
                                                <telerik:RadComboBox runat="server" ID="Jobroles" EmptyMessage="Select a Job Role"
                                                    ...
                                                    Skin="MetroTouch" ShowToggleImage="True" InputCssClass="form-control input-lg" CheckBoxes="true"
                                                    EnableCheckAllItemsCheckBox="true" DataTextField="JobRole">
                                                </telerik:RadComboBox>
                                            </div>
                                       
                                            <div id="Responsibility" class="col-lg-12  w3-animate-left defineBy">
                                                <span class="text-blue pull-left">Responsibilities </span>
                    ...
                    <div class="panel-heading">
                        <h4 class="panel-title">
                            <a id="collapseDragDrop" data-parent="#accordion" class="clickable" href="#DragDrop" data-toggle="collapse"><span id="spanTasks" class="glyphicon glyphicon-plus"></span>
                                <label class="clickable" id="labelAddTasks">Step 2: Add Steps</label>
                            </a>
                        </h4>
                    </div>
                                ...
                                <div class="col-lg-10 col-lg-offset-1"><br />
                                    <div id="row1" ng-repeat="task in tasks" class="input-group input-group-lg addTask w3-animate-bottom">
                                        <input id="{{'input' + $index}}" class="form-control input-lg" taskindex="{{ $index }}" onkeypress="EnterKeyPressed(event, this)" 
                                             ng-model="task.Task" placeholder="Add New Step Here..." ng-keypress="AddTaskElement($index)" />
...
<asp:Content ID="Content3" ContentPlaceHolderID="PageScripts" runat="server">
    ...
    <script>
        ...
        {
            var trainingTypeCombo = $find('<%=RadComboBoxTrainingTypes.ClientID %>');
           
               
            if (typeof angular.element('#CACtrl').scope().defHeaderObj !== "undefined" && typeof angular.element('#CACtrl').scope().SelectedTitles !== "undefined"
                && typeof angular.element('#CACtrl').scope().SelectedRoles !== "undefined" && typeof angular.element('#CACtrl').scope().SelectedResps !== "undefined"
                && trainingTypeCombo.get_items().get_count() > 0) {
               
                var allTitles = angular.element('#CACtrl').scope().SelectedTitles;
                var allRoles =  angular.element('#CACtrl').scope().SelectedRoles;
                var allResps = angular.element('#CACtrl').scope().SelectedResps;                
                var trainingtype = trainingTypeCombo.findItemByValue(angular.element('#CACtrl').scope().defHeaderObj.TrainingTypeID);              
                trainingtype.select();

                <%-- alert(angular.element('#CACtrl').scope().SelectedRoles);
             ...
             <%--   var UOMID = $find('<%=RadComboBoxUOM.ClientID %>');
               ...
               // FUOM.select();
                ...
                if (allTitles != '')
                {
                    ...
                    {
                        var title = TitleID.findItemByValue(allTitles[i]); 
                        title.set_checked(true);
                    }
                }
                if (allRoles != '') {                   
                    ...
                    for (var i = 0; i < allRoles.length; i++) {
                        var role = JobroleID.findItemByValue(allRoles[i]);
                ...
                if (allResps != '') {
                    ...
                    allResps = allResps.split(';');
                    var RespID = $find('<%=RadComboBoxJobResp.ClientID %>');
                  
                    for (var i = 0; i < allResps.length; i++) {
                        var resp = RespID.findItemByValue(allResps[i]);
                        resp.set_checked(true);
                    }
        ...
        $('#saveHeader').click(function (e) {
            ...
            var JobroleID = $find('<%=Jobroles.ClientID %>').get_checkedItems();
            var TitleID = $find('<%=RadComboBoxJobTitles.ClientID %>').get_checkedItems();
            var RespID = $find('<%=RadComboBoxJobResp.ClientID %>').get_checkedItems();
            var UOMID = $find('<%=RadComboBoxUOM.ClientID%>').get_value();
            var FUOMID = $find('<%=RadComboBoxFUOM.ClientID%>').get_value();
            var trainingTypeID = $find('<%=RadComboBoxTrainingTypes.ClientID%>').get_value();
            
            for (i = 0; i < JobroleID.length; i++) {
                jobRoles += JobroleID[i].get_value() + ';';
            };
            for (i = 0; i < TitleID.length; i++) {
                jobTitles += TitleID[i].get_value() + ';';
            };
            for (i = 0; i < RespID.length; i++) {
                jobResp += RespID[i].get_value() + ';';
            };
             ...
             var frecQ = $('#frecQ').val();
             if (validateHeader()) {
                 //'PassingGrade': PassingGrade, 'NotifyDays': NotifyDays, 'NotifyEmail': NotifyEmail, 'RecurrenceQty': recQty, 'UOMSchedulingRecurrenceID': UOMID
                 headerObject = {
                     "CompetencyDefHdrID": 0, "JobTitleID": jobTitles, "JobRoleID": jobRoles, "JobResponsibilityID": jobResp, "Description": Descr, "TrainingTypeID": trainingTypeID
                 };
        ...
        $('#SaveCompelete').click(function () {
         ...
         function validateHeader() {
             ...
             if (JobroleID.length == 0 && RespID.length == 0 && TitleID.length == 0)
             {
