...
.controller('CACtrl', function ($scope, $http, $location, InsertHeader, InsertDetail, RemoveCompetency_fromDefinition, UpdateHeader, AddTask, UpdateItem, updateAllSorts) {
  ...
  , $scope.maxSize = 5;
    ...
    //-------------------Begin: The following function return the roles, responsibilities, and titles set up for the competency definition
    $scope.getSelectedRoles = function (compDefHeader) {      
        $http.get("/Competency/GetCompJobRoles?" + "compHeaderID=" + compDefHeader)
          .success(function (data) {
              var jsonComp = JSON.stringify(data);
              jsonComp = jQuery.parseJSON(jsonComp);
    ...
    $scope.getSelectedResps = function (compDefHeader) {
        $http.get("/Competency/GetCompJobResponsibilities?" + "compHeaderID=" + compDefHeader)
          .success(function (data) {
              var jsonComp = JSON.stringify(data);
              jsonComp = jQuery.parseJSON(jsonComp);
              $scope.SelectedResps = data;
          });
    };
    $scope.getSelectedTitles = function (compDefHeader) {
        $http.get("/Competency/GetCompJobTitles?" + "compHeaderID=" + compDefHeader)
          .success(function (data) {
              var jsonComp = JSON.stringify(data);
              jsonComp = jQuery.parseJSON(jsonComp);
              $scope.SelectedTitles = data;
    ...
    if (typeof $location.search().compHeaderID !== 'undefined') {
        ...
        //The following lines get data for the drop down lists that define the competencies.
        $scope.getSelectedRoles($scope.compHeaderID);
        $scope.getSelectedResps($scope.compHeaderID);
        $scope.getSelectedTitles($scope.compHeaderID);
    ...
    $scope.InsertHeader = function (HeaderObject) {
        ...
        if ($scope.defHeader == 0) {
            InsertHeader.InsertHeader(HeaderObject, $scope);
            
        }
        else {
            UpdateHeader.UpdateHeader(HeaderObject, $scope);
        }
    }
...
.controller('MngCompCtrl', function ($scope, $http, $location, RemoveCompetency_fromDefinition, RemoveCompetencyDefinition) {
    ...
    $scope.HeaderDefObject = [{
        ...
        'PassingGrade': 0, 'NotifyDays': 0, 'Recurrence':'', 'TrainingType': ''
    }];
...
.factory("InsertHeader", ['$http', function ($http, $scope) {
    ...
    fac.InsertHeader = function (headerObj, $scope) {
        ...
        var jsonHeader = JSON.stringify(headerObj);
       
      
        $http.post("/Competency/InsertHeader?" + 'headerObject=' + encodeURIComponent(jsonHeader)).success(function (response) {
...
.factory("UpdateHeader", ['$http', function ($http, $scope) {
    ...
    fac.UpdateHeader = function (headerObj, $scope) {
        ...
        $http.post("/Competency/UpdateHeader?" + 'headerObject=' + encodeURIComponent(jsonHeader)).success(function (response) {
            //$scope.defHeader = response;
            //$scope.$apply();
        })
    }
